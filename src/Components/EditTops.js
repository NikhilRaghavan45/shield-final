...
const EditTops = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:3001/tops/${id}`)
            .then((res) => {
                setProduct(res.data);
            })
            .catch((err) => {
    ...
    const updateProduct = (event) => {
        ...
        axios.put(`http://localhost:3001/tops/${id}`,product)
            .then((res) => {
              window.alert("Tops details updated successfully");
        ...
        <div className='d-flex w-100 vh-100 justify-content-center align-items-center bg-light'>
            <div className='w-50 border bg-white shadow px-5 pt-3 pb-5 rounded'>
                ...
                <form onSubmit={updateProduct}>
                    ...
                    <Link to={`/tops`} className='btn btn-primary ms-3'>Back</Link>
                    </div>
                </form>

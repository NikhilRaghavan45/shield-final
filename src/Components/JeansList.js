...
const PantsList = () => {
    ...
    const handleDelete = (id) => {
        ...
        if (confirm) {
            axios.delete(`http://localhost:3001/pants/${id}`)
                .then(res => {
    ...
    return (
        <div className='container mt-5 '>
            ...
            <div className=' d-flex justify-content-end'>
                <Link to="/addpants" className="btn btn-success">AddProduct
                </Link>{"\n"}
            </div>
                    ...
                    {data.map((d, i) => {
                        return <tr key={i}>
                            ...
                            <td>
                                ...
                                <Link to={`/editpants/${d.id}`} className='btn btn-sm btn-primary me-2'>Edit</Link>

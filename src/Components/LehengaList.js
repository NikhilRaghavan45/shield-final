...
const LehengaList = () => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:3001/lehenga`)
    ...
    const handleDelete = (id) => {
        ...
        if (confirm) {
            axios.delete(`http://localhost:3001/lehenga/${id}`)
                .then(res => {
    ...
    return (
        <div className='container mt-5 '>
            ...
            <div className=' d-flex justify-content-end'>
                <Link to="/addlehenga" className="btn btn-success">AddProduct
                </Link>{"\n"}
            </div>
                    ...
                    {data.map((d, i) => {
                        return <tr key={i}>
                            ...
                            <td>
                                ...
                                <Link to={`/editlehenga/${d.id}`} className='btn btn-sm btn-primary me-2'>Edit</Link>
